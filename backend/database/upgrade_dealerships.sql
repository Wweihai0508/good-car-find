...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''dealership_id column already exists''');
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''location_address column already exists''');
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''contact_person column already exists''');
...
SET @col_exists = (SELECT COUNT(*) FROM information_schema.columns
                 WHERE table_schema = 'railway'
                 AND table_name = 'cars'
                 AND column_name = 'contact_wechat');

SET @sql = IF(@col_exists = 0,
    'ALTER TABLE cars ADD COLUMN contact_wechat VARCHAR(100) COMMENT ''微信'''',
    'SELECT ''contact_wechat column already exists''');
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''distance_km column already exists''');
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''car_source column already exists''');
...
SET @fk_exists = (SELECT COUNT(*) FROM information_schema.table_constraints
                 ...
                 AND table_name = 'cars'
                 AND constraint_name = 'cars_ibfk_2');

SET @sql = IF(@fk_exists = 0,
    'ALTER TABLE cars ADD CONSTRAINT cars_ibfk_2 FOREIGN KEY (dealership_id) REFERENCES dealerships(id) ON DELETE SET NULL',
